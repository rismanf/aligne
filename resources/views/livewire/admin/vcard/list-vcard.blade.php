<div>
    <x-card>
        <div class="flex justify-end mb-4">
            ...
            <x-button label="Add" icon="o-plus" onclick="window.location.href='{{ route('admin.vcard.create') }}'"
                class="btn-primary btn-xs p-2" />
        </div>

        <x-hr target="gotoPage" />
        <x-table class="text-xs" :headers="$t_headers" :rows="$vcards" >
            {{-- Special `row_number` scope --}}

            {{-- Special `actions` slot --}}
            @scope('cell_action', $vcards)
                <div class="flex gap-1">
                  <x-button icon="o-pencil" onclick="window.location.href='{{ route('admin.vcard.edit', $vcards->id) }}'"
                        spinner class="btn-xs" />
                    <x-button icon="o-eye" onclick="window.open('{{ route('admin.news.show', $vcards->id) }}', '_blank')"
                        spinner class="btn-xs" />
                    <x-button icon="o-trash" wire:click="showDeleteModal({{ $vcards->id }})" spinner class="btn-xs" />
                </div>
            @endscope
        </x-table>
    </x-card>

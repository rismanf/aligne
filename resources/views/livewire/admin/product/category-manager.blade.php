    ...
    <x-card>
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex gap-2">
                <x-button label="Back to Packages" icon="o-arrow-left" onclick="window.location.href='{{ route('admin.product.index') }}'" class="btn-outline" />
...
{{--         
        @if($categories_data) --}}
            <x-table class="text-xs" :headers="$t_headers" :rows="$categories_data">
                {{-- Status scope --}}
                ...
                @scope('cell_action', $categories_data)
                    <div class="flex gap-1">
                        <x-button icon="o-pencil" wire:click="showEditModal({{ $categories_data->id }})" spinner class="btn-xs btn-warning" tooltip="Edit Category" />
                        <x-button icon="@if($categories_data->is_active) o-eye-slash @else o-eye @endif" 
                                  wire:click="toggleStatus({{ $categories->id }})" 
                                  spinner 
                                  class="btn-xs @if($categories_data->is_active) btn-warning @else btn-success @endif" 
                                  tooltip="@if($categories_data->is_active) Deactivate @else Activate @endif" />
                        <x-button icon="o-trash" wire:click="showDeleteModal({{ $categories_data->id }})" spinner class="btn-xs btn-error" tooltip="Delete Category" />
                    </div>
                @endscope
            </x-table>
        {{-- @else
    ...
    <x-modal wire:model="createForm" title="Create New Category" class="backdrop-blur">
        <x-form wire:submit="save">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-input label="System Name" wire:model="name" placeholder="e.g., signature" hint="Lowercase, no spaces" />
            ...
            <x-textarea label="Description" wire:model="description" placeholder="Brief description of this category..." rows="3" />
    ...
    <x-modal wire:model="deleteForm" title="Delete Category" subtitle="This action cannot be undone.">
        <div class="alert alert-warning">
            ...
            <span>Warning: This will permanently delete the category. Make sure no products are using this category!</span>
        </div>
